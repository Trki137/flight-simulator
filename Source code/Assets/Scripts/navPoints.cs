...
public class navPoints : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject G = GameObject.Find("Config");
        ReadConfigFile configFile = G.GetComponent<ReadConfigFile>();

        numOfNavPoints = configFile.getNumOfNavPoints();
    ...
    {
           for(int i = 0; i < numOfNavPoints; i++) {
             float X;
             float Y;
            ...
            GameObject tocka = Instantiate(prefab, position, prefab.transform.rotation);
           ...
           // tocka.GetComponent <TMPro.TextMeshProUGUI> ().text +=i;
            ...
            tekst.text+=(i+1).ToString();

            string log = string.Format("[{0}] Navigation point {1} is at position ({2},{3})", dateTime ,i + 1, position.x, position.y);
            writeLog.writeLog(log);

            positionOfAllNavPoints.Add(position);
